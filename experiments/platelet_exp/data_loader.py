...
def create_data_gen_pipeline(patient_data, cf, do_aug=True):
    ...
    my_transforms.append(ConvertSegToBoundingBoxCoordinates(cf.dim, get_rois_from_seg_flag=GET_ROIS_FROM_SEG, class_specific_seg_flag=cf.class_specific_seg_flag))
...
class BatchGenerator(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        batch_ixs = [0] * self.batch_size #get_batch_ixs(len(class_targets_list), self.batch_size)
        ...
        for i in range(seg.shape[0]):
            if len(class_target[i]) == 1:
                class_target[i] *= seg[i].max()
                if seg[i].max() > 500 or len(class_target[i]) > 500:
                    print(f'WARNING: got max {seg[i].max()} and length {len(class_target[i])}')
...
class PatientBatchIterator(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        batch_3D = {'data': out_data, 'seg': out_seg, 'class_target': batch_class_targets, 'pid': pid}
        converter = ConvertSegToBoundingBoxCoordinates(dim=3, get_rois_from_seg_flag=GET_ROIS_FROM_SEG, class_specific_seg_flag=self.cf.class_specific_seg_flag)
...
def get_batch_ixs(n_vols, batch_size):
    ...
    if batch_size <= n_vols:
        ...
        return list(np.random.choice(range(n_vols), size=n_vols, replace=False)) + \
            get_batch_ixs(n_vols, batch_size - n_vols)
